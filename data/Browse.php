...
class Browse implements BrowseStrategy {
    ...
    public static function getFieldsTranslations($entity, $langorder = null) {
        ...
        foreach ($res as $record) {
            if (isset($results[$record["field_number"]])) {
                $results[$record["field_number"]]["label_" . $record["locale"]] = $record["msgstr"];
            } else {
                $results[$record["field_number"]] = $record;
                if ($record["locale"] != "en") {
                    $results[$record["field_number"]]["label_" . $record["locale"]] = $record["msgstr"];
                }
            }
    ...
    public static function getFieldByName($entity, $field_name) {
        $browse = new Browse();
        $res = $browse->ExecuteQuery("SELECT field_number,field_name , essential FROM data_dict WHERE entity='$entity' and field_name='$field_name' and enabled='y'");
        return $res;
    }
...
//        $sql = "SELECT * FROM help WHERE field_number = $id ";
    ...
    public static function getHelpText($entity, $langorder = null) {
        global $global;
        global $conf;
        ...
        $sql = "SELECT 
                    ...
                    h.entry as entry_en,
                    h.examples as examples_en,
                    l.definition as definition,
                    l.guidelines as guidelines,
                    l.entry as entry,
                    l.examples as examples,
                    l.locale
                FROM data_dict AS d
        ...
        foreach ($res as $record) {
            if (isset($results[$record["field_number"]])) {
                $results[$record["field_number"]]["definition_" . $record["locale"]] = $record["definition"];
                $results[$record["field_number"]]["guidelines_" . $record["locale"]] = $record["guidelines"];
                $results[$record["field_number"]]["entry_" . $record["locale"]] = $record["entry"];
                $results[$record["field_number"]]["examples_" . $record["locale"]] = $record["examples"];
            } else {
                $results[$record["field_number"]] = $record;
                if ($record["locale"] != "en") {
                    $results[$record["field_number"]]["definition_" . $record["locale"]] = $record["definition"];
