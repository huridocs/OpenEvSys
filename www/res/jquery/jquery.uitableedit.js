...
jQuery.uiTableEdit = function(jq, options){
  ...
  function td_edit(){
    var td = jQuery(this);
    if(td.attr('class')=='no_edit')return;

    function restore(e){
      var val = td.find(':text').attr('value')
      if( options.dataVerify ){
        var msgid = td.attr('id');
        var lan = td.attr('class');
        var value = options.dataVerify.call(this, val, orig_text , msgid , lan , e, td);
    ...
    td.text() + '"' + ' style="margin:0px;background:#FFFFCC;padding:0px;border:0px;width: ' +
      w  + 'px;">' + '</input></form>' )
