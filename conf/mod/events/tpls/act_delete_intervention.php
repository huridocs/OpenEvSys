...
<?php if($del_confirm){ ?>
    ...
    <form class="form-horizontal"  action="<?php get_url('events','delete_intervention')?>" method="post">
        ...
        <?php			
			foreach($intervention_ids as $val){ ?>
...
?>
<center>
	<a class="btn" href="<?php echo get_url('events','intv_list',null,array('eid'=>$event_id)) ?>"><i class="icon-chevron-left"></i> <?php echo _t('BACK')?></a>
</center>
<br />
</div>
